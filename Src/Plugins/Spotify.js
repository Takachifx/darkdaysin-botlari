...
module.exports = class Spotify {
  constructor(options) {
    ...
    this.randomColors = [Discord.Colors];
  }
 ...
 return this;
  ...
  async build() {
    ...
    const start_format = moment.duration(this.start > this.end ? this.end : this.start).format("mm:ss");
    const end_format = moment.duration(this.end).format("mm:ss");
    ...
    if(this.spotifyLogo){
    try {
      ctx.drawImage(await loadImage(`${__dirname}/../İmages/spotify-logo.png`), 950, 60, 100, 100);
    } catch {
    ...
    if (this.album && typeof this.album === "string") {
      ctx.fillStyle = "#020202";
      ctx.font = `regular 40px ${this.font.name}`;
      ctx.fillText(`${this.album.length >= 40 ? this.album.slice(0, 40)+"...":this.album}`, 1000, 350);
    }

    ctx.fillStyle = "#020202";
    ctx.font = `"regular 40px ${this.font.name}`
    ctx.fillText(end_format, 1782, 416);

    ctx.fillStyle = "#020202";
    ctx.font = `regular 40px ${this.font.name}`;
    ctx.fillText(start_format, 230, 416);
