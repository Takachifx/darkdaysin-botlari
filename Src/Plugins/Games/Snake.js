...
const editEmbed = new Discord.EmbedBuilder().setColor("Green")
let embedicerik = options.embed.description.replace('{{score}}', score).replace('{{kazanilan}}', Number(score * 10 * 5)).replace('{{author}}', options.message.author)
if(score > 0) await Dolars.updateOne({ guildID: settings.Moderation.guildID, userID: options.message.author.id }, { $inc: { dolar: Number(score * 10 * 5) } }, { upsert: true });
...
await Dolars.updateOne({ guildID: settings.Moderation.guildID, userID: options.message.author.id }, { $inc: { dolar: -options.bahis } }, { upsert: true })
