...
const emojis = require("../../Settings/emojiName.json")
const Dolars = require("../../Schemas/Dolars")
...
options.embed.footer = options.message.member.username;
}
...
return options.message.react(options.message.guild.emojiGöster(emojis.no)).catch(err => {});
}
...
if (oppenent.bot || oppenent.id === challenger.id) return options.message.react(options.message.guild.emojiGöster(emojis.no)).catch(err => {});
let acceptbutton = new Discord.ButtonBuilder()
.setStyle(Discord.ButtonStyle.Secondary)
.setLabel(options.buttons.accept)
.setCustomId("accept")
.setEmoji(options.message.guild.emojiGöster(emojis.yes).id);
let denybutton = new Discord.ButtonBuilder()
.setStyle(Discord.ButtonStyle.Danger)
.setLabel(options.buttons.deny)
.setCustomId("deny")
.setEmoji(options.message.guild.emojiGöster(emojis.no).id);;
...
Collector.stop();
await Dolars.updateOne({guildID: settings.Moderation.guildID, userID: challenger.id}, {$inc: {dolar: -options.bahis}, }, {upsert: true})
await Dolars.updateOne({guildID: settings.Moderation.guildID, userID: oppenent.id}, {$inc: {dolar: -options.bahis}, }, {upsert: true})
...
.setDescription(
`**Kazanılacak Ödül**: \` ${options.ödül} ${options.parabirim} Doları \`
Sıra sende **${gameData[tempPlayer].member.user}**

${gameData[player].member} (${gameData[player].health} HP) (:punch:)
**${gameData[tempPlayer].member.user}** (${gameData[tempPlayer].health} HP)

${gameData[player].member} Üyesi Saldırıyı Seçti!
Savaş Devam Ediyor...`,
)
...
.setDescription(
`**Kazanılacak Ödül**: \` ${options.ödül} ${options.parabirim} Doları \`
Sıra sende **${gameData[tempPlayer].member}**

**${gameData[tempPlayer].member}** (${gameData[tempPlayer].health} HP)
${gameData[player].member.user} (${gameData[player].health} HP) (:punch:)

${gameData[player].member.user} Üyesi Saldırıyı Seçti!
Savaş Devam Ediyor...
`,
)
...
${gameData[player].member} (${gameData[player].health} HP + :hearts:)
**${gameData[tempPlayer].member.user}** (${gameData[tempPlayer].health} HP)

${gameData[player].member} Üyesi Savun Ve İyileşi Seçti!
Savaş Devam Ediyor...`,
)
...
**${gameData[tempPlayer].member}** (${gameData[tempPlayer].health} HP)
${gameData[player].member.user} (${gameData[player].health} HP + :hearts:)

${gameData[player].member.user} Üyesi Savun Ve İyileşi Seçti!
Savaş Devam Ediyor...`,
)
...
.setColor(options.embed.color);
await Dolars.updateOne({guildID: settings.Moderation.guildID, userID: gameData[player].member.id}, {$inc: {dolar: options.ödül}, }, {upsert: true});
question.edit({
embeds: [__embed],
components: [],
});
...
.setColor(options.embed.color);
await Dolars.updateOne({guildID: settings.Moderation.guildID, userID: gameData[tempPlayer].member.id}, {$inc: {dolar: options.ödül}, }, {upsert: true});
