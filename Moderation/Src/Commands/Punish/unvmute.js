...
conf: {
name: "unvmute",
aliases: ["unvmute", "unvoice-mute", "unvoice-sustur", "sesli-sustur-kaldır", "sesli-mute-kaldır"],
help: "unvmute @Darkdays/ID",
...
if(!ayar) return;
const Name = ["unvmute", "unvoice-mute", "unvoice-sustur", "sesli-sustur-kaldır", "sesli-mute-kaldır"];
...
await member.voice.setMute(false).catch(e => {});
const data = await Penalties.findOne({ userID: member.user.id, guildID: settings.Moderation.guildID, type: "VOICE-MUTE", active: true });
if (data) {
data.active = false;
await data.save();
}
await message.reply({ content:`${message.guild.emojiGöster(emojis.punish)} ${member.toString()} Üyesinin **Ses Kanallarındaki** Susturması <t:${String(Date.now()).slice(0, 10)}:R> ${message.member.toString()} Tarafından Kaldırıldı!`})
const log = embed
...
.setDescription(`${message.guild.emojiGöster(emojis.info)} ${member.toString()} Adlı Kullanıcının **Ses Kanallarındaki** Susturması ${message.member.toString()} Tarafından Kaldırıldı.
