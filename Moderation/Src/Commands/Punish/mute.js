...
conf: {
name: "mute",
aliases: ["cmute", "chatmute", "sustur", "susturma", "chat-mute", "chat-sustur"],
help: "mute @Darkdays/ID Süre Sebep",
...
if(!ayar) return;
const Name = ["cmute", "chatmute", "sustur", "susturma", "chat-mute", "chat-sustur"];
...
return }
const data = await Penalties.findOne({ userID: member.user.id, guildID: settings.Moderation.guildID, $or: [{ type: "CHAT-MUTE" }], active: true });
if (data) {
await message.react(message.guild.emojiGöster(emojis.no))
await message.reply({ content:"Bu Kullanıcı Zaten Susturulmuş!"}).sil(15)
return }
...
return }
await message.react(message.guild.emojiGöster(emojis.yes))
await member.addRoles(ayar.muteRoles).catch(e => {});
const penal = await client.Punished(settings.Moderation.guildID, member.user.id, "CHAT-MUTE", true, message.author.id, reason, true, Date.now() + duration);
const cezas = await client.kanalBul("ceza-log");
...
await message.reply({ content:`${message.guild.emojiGöster(emojis.punish)} ${member.toString()} Kullanıcısı <t:${String(Date.now()).slice(0, 10)}:R> " **${reason}** " Sebebiyle ${message.member.toString()} Tarafından **Text Kanallarında** \` ${time} \` Susturuldu. Ceza Numarası: **(${penal.id})**`})
const log = embed
.setColor("Random")
...
.setDescription(`${message.guild.emojiGöster(emojis.info)} ${member.toString()} Kullanıcısı ${message.member.toString()} Tarafından **Text Kanallarında** Susturuldu.
