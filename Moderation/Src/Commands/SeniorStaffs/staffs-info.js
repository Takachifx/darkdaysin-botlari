...
module.exports = {
conf: {
aliases: ["staffs-info", "staffsinfo", "staff-info", "staffinfo", "ysay", "yetkilisay"],
name: "staffs-info",
help: "yetkilisay",
category: "ustyetkili",
cooldown: 15
...
if(!ayar) return;
const Name = ["staffs-info", "staffsinfo", "staff-info", "staffinfo", "ysay", "yetkilisay"];
...
const SesteOlmayanYetkili = message.guild.members.cache.filter(x => !x.user.bot && ayar.staffRoles.some(t => x.presence && x.presence.status !== "offline" && x.roles.cache.has(t))).filter(x => !x.voice.channel);
const AktifYetkili = message.guild.members.cache.filter(x => !x.user.bot && ayar.staffRoles.some(t => x.roles.cache.has(t)) && x.presence && x.presence.status !== "offline").size;
const OfflineYetkili = message.guild.members.cache.filter(x => !x.user.bot && ayar.staffRoles.some(t => x.roles.cache.has(t))).filter(x => !x.presence || x.presence.status === "offline").size;
const SesCagir = message.guild.members.cache.filter(x => !x.user.bot && ayar.staffRoles.some(t => x.roles.cache.has(t)) && x.presence && x.presence.status !== "offline").filter(x => !x.voice.channel);
const Yetkililer = message.guild.members.cache.filter(x => !x.user.bot && ayar.staffRoles.some(t => x.roles.cache.has(t)));
let Text = `${message.guild.emojiGöster(emojis.face)} Sunucumuzda Toplam **${StaffsSize}** Yetkili Bulunmakta!\n\n${message.guild.emojiGöster(emojis.sagok)} Seste Olan Yetkili Sayısı: **${SesteOlanYetkili}**\n${message.guild.emojiGöster(emojis.sagok)} Seste Olmayan Yetkili Sayısı: **${SesteOlmayanYetkili.size}**\n${message.guild.emojiGöster(emojis.sagok)} Aktif Yetkili Sayısı: **${AktifYetkili}**\n${message.guild.emojiGöster(emojis.sagok)} Aktif Olmayan Yetkili Sayısı: **${OfflineYetkili}**\n\n${message.guild.emojiGöster(emojis.sagok)} Sunucudaki Yetkililer;\n${Yetkililer.map(x => `${x} - **${x.roles.highest.name}**`).join("\n")}\n\n${message.guild.emojiGöster(emojis.sagok)} Seste Olmayan Yetkililer;\n${SesCagir.map(x => `${x}`).join("\n")}`;
...
await button.deferUpdate();
message.guild.members.cache.filter(yetkili => ayar.staffRoles.some(t => yetkili.roles.cache.has(t)))
.filter(yetkilises => !yetkilises.user.bot && !yetkilises.voice.channel)
.forEach(user => {
user.send({ content: `${user} Merhabalar. **${message.guild.name}** sunucusunda ses aktifliğinizi artırmak ve yetkinizi yükseltmek için seslere giriniz. Müsait değilsen **Sleep Room** kanalına afk bırakabilirsin.` }).catch(err => {
message.channel.send({ content: `${user} isimli yetkiliye özel mesajları kapalı olduğu için mesaj atamıyorum. Lütfen seslere geçebilir misin? Müsait değilsen **Sleep Room** kanalına geçebilirsin.` });
});
});
}
});
