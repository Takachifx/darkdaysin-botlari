...
module.exports = {
conf: {
aliases: ["yetkili", "yetkibaslat", "yetkibaşlat", "ytb"],
name: "staff-add",
help: "yetkili @Darkdays/ID",
category: "yetkili",
cooldown: 60,
},
...
if(ayar.tagSystem == true && !ayar.serverTag.some(s => member.user.globalName && member.user.globalName.includes(s) || member.user.username.includes(s))) {
await message.react(message.guild.emojiGöster(emojis.no))
...
if(kontrol && kontrol.Staff == true) {
await message.react(message.guild.emojiGöster(emojis.no))
await message.reply({content: `${member} isimli üye zaten bir başkası tarafından yetkili olarak belirlenmiş.`}).sil(15)
return }
...
await Puans.updateOne({ guildID: settings.Moderation.guildID, userID: message.author.id }, { $inc: { puan: rdata.staffCoin } }, { upsert: true });
message.member.görevGüncelle(settings.Moderation.guildID, "yetkili", 1, message.channel);
await Puans.updateOne({ guildID: settings.Moderation.guildID, userID: member.id }, { $inc: { puan: 2 } }, { upsert: true });
ayar.staffStartRoles.map(async (s) => { await member.addRoles(s) })
...
channel.send({embeds: [embed.setDescription(`${member.toString()} Üyesi ${message.author} Tarafından <t:${String(Date.now()).slice(0, 10)}:R> Yetkili Olarak Belirlendi!`).setFooter({ text: `Yetki Verildi`, iconURL: message.author.avatarURL({ dynamic: true })})]})
...
Embeds.setTimestamp()
Embeds.setDescription(`${message.guild.emojiGöster(emojis.no)} ${message.author} ${member} Adlı Kullanıcı Senin Yetki Aldırma İsteğini Reddetti.`)
